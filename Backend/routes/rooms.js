  ...
  try {
    ...
    const result = await Promise.all(Array.from(roomIds).map(async (rid) => {
      ...
      return {
        ...
        createdBy: (roomMeta && roomMeta.createdBy) ? roomMeta.createdBy.username : 'Unknown',
        isCreator: (roomMeta && roomMeta.createdBy) ? roomMeta.createdBy._id.toString() === req.user.id : false,
