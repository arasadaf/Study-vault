    ...
    <div className="flex-1 flex flex-col p-6 overflow-y-auto bg-white/30">
      <div className="max-w-3xl mx-auto w-full space-y-8">
        ...
        <div className="space-y-6">
          ...
          {doubts.length === 0 ? (
            <div className="text-center py-12 text-slate-600">No doubts posted yet.</div>
          ) : (
            doubts.map(doubt => (
              <div key={doubt._id} className={`glass-panel p-6 rounded-2xl border transition-all ${doubt.isResolved ? 'border-green-500/20' : 'border-slate-900/10'}`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-lg font-bold text-slate-900 mb-1">{doubt.question}</h4>
                    <p className="text-sm text-slate-600 mb-3">{doubt.description}</p>
                    <div className="flex items-center gap-3 text-[10px] uppercase font-bold tracking-wider">
                      <span className="text-amber-600">Asked by {doubt.askedBy}</span>
                ...
                {/* Answers Section */}
                <div className="mt-6 pt-6 border-t border-slate-900/10 space-y-4">
                  <h5 className="text-sm font-bold text-slate-700">Answers ({doubt.answers.length})</h5>
                  {doubt.answers.map(ans => (
                    <div key={ans._id} className={`p-4 rounded-xl text-sm ${ans.isAccepted ? 'bg-green-500/10 border border-green-500/20' : 'bg-white/50 border border-transparent'}`}>
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-slate-800 mb-2">{ans.answer}</p>
                          <span className="text-[10px] text-slate-600">Answered by {ans.answeredBy}</span>
                        </div>
                        {ans.isAccepted && (
                        ...
                        {!doubt.isResolved && JSON.parse(localStorage.getItem('vault_user'))?.username === doubt.askedBy && (
                          <button 
                            ...
                            className="text-[10px] font-bold text-amber-600 hover:text-amber-700 px-2 py-1 border border-amber-600/30 rounded"
                          >
                            Accept Answer
                          </button>
                        )}
